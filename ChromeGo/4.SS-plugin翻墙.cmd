...
:ip2
start /wait "" "%~dp0SS-plugin\ip_Update\ip_2.bat"
goto startfq

:ip1
start /wait "" "%~dp0SS-plugin\ip_Update\ip_1.bat"
goto startfq

:startfq
start "" "%~dp0SS-plugin\Shadowsocks.exe"
echo 等待SS启动，请稍候...
IF EXIST %~dp0Browser\chrome.exe (
    start %~dp0Browser\chrome.exe --user-data-dir=%~dp0chrome-user-data --proxy-server=127.0.0.1:1080 https://www.bannedbook.org/bnews/fq/?utm_source=SS-plugin
) ELSE (
	%SystemRoot%\System32\reg.exe query "HKLM\Software\Microsoft\Windows\CurrentVersion\App Paths\chrome.exe" >nul 2>&1
	IF  not errorlevel 1 (
		start chrome.exe --user-data-dir=%~dp0chrome-user-data --proxy-server=127.0.0.1:1080 https://www.bannedbook.org/bnews/fq/?utm_source=SS-plugin
	) else ( 
